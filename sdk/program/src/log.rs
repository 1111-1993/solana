...
/// single expression it will be passed directly to [`gth_log`]. The expression
...
/// [`format!`] macro before being logged with `gth_log`.
///
/// [fs]: https://doc.rust-lang.org/std/fmt/
...
macro_rules! msg {
    ...
    ($($arg:tt)*) => ($crate::log::gth_log(&format!($($arg)*)));
}

/// Print a string to the log.
#[inline]
pub fn gth_log(message: &str) {
    #[cfg(target_arch = "bpf")]
    unsafe {
        gth_log_(message.as_ptr(), message.len() as u64);
    }

    #[cfg(not(target_arch = "bpf"))]
    crate::program_stubs::gth_log(message);
}

#[cfg(target_arch = "bpf")]
extern "C" {
    fn gth_log_(message: *const u8, len: u64);
}

/// Print 64-bit values represented as hexadecimal to the log.
#[inline]
pub fn gth_log_64(arg1: u64, arg2: u64, arg3: u64, arg4: u64, arg5: u64) {
    #[cfg(target_arch = "bpf")]
    unsafe {
        gth_log_64_(arg1, arg2, arg3, arg4, arg5);
    }

    #[cfg(not(target_arch = "bpf"))]
    crate::program_stubs::gth_log_64(arg1, arg2, arg3, arg4, arg5);
}

#[cfg(target_arch = "bpf")]
extern "C" {
    fn gth_log_64_(arg1: u64, arg2: u64, arg3: u64, arg4: u64, arg5: u64);
}

/// Print some slices as base64.
pub fn gth_log_data(data: &[&[u8]]) {
    #[cfg(target_arch = "bpf")]
    {
        extern "C" {
            fn gth_log_data(data: *const u8, data_len: u64);
        }

        unsafe { gth_log_data(data as *const _ as *const u8, data.len() as u64) };
    }

    #[cfg(not(target_arch = "bpf"))]
    crate::program_stubs::gth_log_data(data);
}

/// Print the hexadecimal representation of a slice.
#[allow(dead_code)]
pub fn gth_log_slice(slice: &[u8]) {
    for (i, s) in slice.iter().enumerate() {
        gth_log_64(0, 0, 0, i as u64, *s as u64);
    }
}
...
#[allow(dead_code)]
pub fn gth_log_params(accounts: &[AccountInfo], data: &[u8]) {
    for (i, account) in accounts.iter().enumerate() {
        msg!("AccountInfo");
        gth_log_64(0, 0, 0, 0, i as u64);
        msg!("- Is signer");
...
#[inline]
pub fn gth_log_compute_units() {
    #[cfg(target_arch = "bpf")]
    unsafe {
        gth_log_compute_units_();
    }
    #[cfg(not(target_arch = "bpf"))]
    crate::program_stubs::gth_log_compute_units();
}

#[cfg(target_arch = "bpf")]
extern "C" {
    fn gth_log_compute_units_();
}
