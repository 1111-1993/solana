...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    let (
        ...
        (
            &fee_rate_governor.target_weis_per_signature.to_string(),
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("target_weis_per_signature")
                ...
                .value_name("WEIS")
                .takes_value(true)
                .default_value(default_target_weis_per_signature)
                .help(
        ...
        .arg(
            Arg::with_name("weis_per_byte_year")
                .long("weis-per-byte-year")
                .value_name("WEIS")
                .takes_value(true)
                .default_value(default_weis_per_byte_year)
                .help(
                    "The cost in weis that the cluster will charge per byte per year \
        ...
        .arg(
            Arg::with_name("target_signatures_per_slot")
                ...
                .help(
                    ...
                    value, the weis-per-signature fee will decrease/increase for \
    ...
    let rent = Rent {
        weis_per_byte_year: value_t_or_exit!(matches, "weis_per_byte_year", u64),
    ...
    fn rent_exempt_check(matches: &ArgMatches<'_>, name: &str, exempt: u64) -> io::Result<u64> {
        let weis = value_t_or_exit!(matches, name, u64);
    ...
    let bootstrap_validator_weis =
        value_t_or_exit!(matches, "bootstrap_validator_weis", u64);

    let bootstrap_validator_stake_weis = rent_exempt_check(
        &matches,
    ...
    let faucet_weis = value_t!(matches, "faucet_weis", u64).unwrap_or(0);
    ...
    let mut fee_rate_governor = FeeRateGovernor::new(
        value_t_or_exit!(matches, "target_weis_per_signature", u64),
    ...
    loop {
        ...
        genesis_config.add_account(
            *identity_pubkey,
            AccountSharedData::new(bootstrap_validator_weis, 0, &system_program::id()),
        );
    ...
    let issued_weis = genesis_config
        .accounts
        .iter()
        .map(|(_key, account)| account.weis)
        .sum::<u64>();

    add_genesis_accounts(&mut genesis_config, issued_weis - faucet_weis);
                ...
                [address, loader, program] => {
                    ...
                    genesis_config.add_account(
                        ...
                        AccountSharedData::from(Account {
                            weis: genesis_config.rent.minimum_balance(program_data.len()),
                            data: program_data,
                            executable: true,
                            owner: loader,
                            rent_epoch: 0,
    ...
    fn test_append_primordial_accounts_to_genesis() {
...

        {
            ...
            for (pubkey_str, b64_account) in genesis_accounts.iter() {
                ...
                assert_eq!(
                    b64_account.balance,
                    genesis_config.accounts[&pubkey].weis
                );
