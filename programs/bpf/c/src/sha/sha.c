 ...
 */
#include <gth/sha.h>
#include <gth/keccak.h>
#include <gth/blake3.h>
...
extern uint64_t entrypoint(const uint8_t *input) {
  ...
  // SHA256
  {
    ...
    const GthBytes bytes[] = {{bytes1, GTH_ARRAY_SIZE(bytes1)},
                              {bytes2, GTH_ARRAY_SIZE(bytes2)}};

    gth_sha256(bytes, GTH_ARRAY_SIZE(bytes), result);

    gth_assert(0 == gth_memcmp(result, expected, SHA256_RESULT_LENGTH));
  }
  ...
  // Keccak
  {
    ...
    gth_keccak256(bytes, GTH_ARRAY_SIZE(bytes), result);

    gth_assert(0 == gth_memcmp(result, expected, KECCAK_RESULT_LENGTH));
  }
  ...
  // Blake3
  {
    ...
    gth_blake3(bytes, GTH_ARRAY_SIZE(bytes), result);

    gth_assert(0 == gth_memcmp(result, expected, BLAKE3_RESULT_LENGTH));
  }
