...
const READONLY_ACCOUNTS: &[GthAccountInfo] = &[
    GthAccountInfo {
        is_signer: false,
        is_writable: false,
        executable: true,
...
fn check_preconditions(
    ...
    for (in_info, static_info) in in_infos.iter().zip(static_infos) {
        ...
        check!(
            unsafe { *in_info.weis.as_ptr() as *const u64 as u64 },
            static_info.weis_addr
        );
        check!(
...
fn process_instruction(
    ...
    match instruction_data[0] {
        1 => {
            ...
            let metas = &[GthAccountMeta {
                is_signer: true,
                is_writable: true,
                pubkey_addr: accounts[1].key as *const _ as u64,
            }];
            let instruction = GthInstruction {
