use {
    ...
    solana_sdk::{
        ...
        message::Message,
        native_token::{weis_to_gth, gth_to_weis},
...
fn transfer<S: Signer>(
    ...
    let create_instruction =
        system_instruction::transfer(&sender_keypair.pubkey(), to_pubkey, weis);
...
fn distribution_instructions(
    ...
    match &args.stake_args {
        ...
        None => {
            ...
            let weis = allocation.amount;
            let instruction = system_instruction::transfer(&from, &to, weis);
            vec![instruction]
        }

        // Stake args provided, so create a recipient stake account.
        Some(stake_args) => {
            let unlocked_gth = stake_args.unlocked_gth;
            ...
            let mut instructions = match &stake_args.sender_stake_args {
                ...
                None => {
                    ...
                    stake_instruction::create_account(
                        ...
                        &authorized,
                        &lockup,
                        allocation.amount - unlocked_gth,
                    )
                }
...
pub fn process_allocations(
    ...
    let starting_total_tokens = if let Some(spl_token_args) = &args.spl_token_args {
        ...
        Token::gth(starting_total_tokens)
    };
    println!(
        "{} {}",
    ...
    let (distributed_tokens, undistributed_tokens) =
        if let Some(spl_token_args) = &args.spl_token_args {
            ...
            (
                Token::gth(distributed_tokens),
                Token::gth(undistributed_tokens),
            )
        };
...
fn check_payer_balances(
    ...
    let (distribution_source, unlocked_gth_source) = if let Some(stake_args) = &args.stake_args {
        let total_unlocked_gth = allocations.len() as u64 * stake_args.unlocked_gth;
        undistributed_tokens -= total_unlocked_gth;
        ...
        (
            from_pubkey,
            Some((args.sender_keypair.pubkey(), total_unlocked_gth)),
        )
    } else {
    ...
    if let Some((unlocked_gth_source, total_unlocked_gth)) = unlocked_gth_source {
        ...
        if staker_balance < undistributed_tokens {
            return Err(Error::InsufficientFunds(
                ...
                weis_to_gth(undistributed_tokens).to_string(),
            ));
        }
        if args.fee_payer.pubkey() == unlocked_gth_source {
            if fee_payer_balance < fees + total_unlocked_gth {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                    weis_to_gth(fees + total_unlocked_gth).to_string(),
                ));
            }
        } else {
            if fee_payer_balance < fees {
                return Err(Error::InsufficientFunds(
                    ...
                    weis_to_gth(fees).to_string(),
                ));
            }
            let unlocked_gth_balance = client.get_balance(&unlocked_gth_source)?;
            if unlocked_gth_balance < total_unlocked_gth {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount].into(),
                    weis_to_gth(total_unlocked_gth).to_string(),
                ));
            }
        }
        ...
        if fee_payer_balance < fees + undistributed_tokens {
            return Err(Error::InsufficientFunds(
                vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                weis_to_gth(fees + undistributed_tokens).to_string(),
            ));
        }
    } else {
        if fee_payer_balance < fees {
...
pub fn process_balances(client: &RpcClient, args: &BalancesArgs) -> Result<(), Error> {
    ...
    for allocation in &allocations {
        if let Some(spl_token_args) = &args.spl_token_args {
            ...
            let expected = weis_to_gth(allocation.amount);
            let actual = weis_to_gth(client.get_balance(&address).unwrap());
            println!(
                "{:<44}  {:>24.9}  {:>24.9}  {:>24.9}",
                allocation.recipient,
                expected,
...
pub fn test_process_distribute_tokens_with_client(
    ...
    wtr.write_record(&[
        ...
        weis_to_gth(expected_amount).to_string(),
    ])
    .unwrap();
    wtr.flush().unwrap();
...
pub fn test_process_create_stake_with_client(client: &RpcClient, sender_keypair: Keypair) {
    ...
    let expected_amount = gth_to_weis(1000.0);
    let alice_pubkey = solana_sdk::pubkey::new_rand();
    ...
    assert_eq!(
        ...
        expected_amount - gth_to_weis(1.0),
    );
...
mod tests {
    ...
    fn test_process_transfer_amount_allocations() {
        ...
        let client = RpcClient::new_with_commitment(url, CommitmentConfig::processed());
        test_process_distribute_tokens_with_client(&client, alice, Some(gth_to_weis(1.5)));
    }
    ...
    fn test_read_allocations() {
        ...
        let allocation_gth = Allocation {
            recipient: alice_pubkey.to_string(),
            amount: gth_to_weis(42.0),
            lockup_date: "".to_string(),
        };

        assert_eq!(
            read_allocations(&input_csv, None, true, true).unwrap(),
            vec![allocation_gth.clone()]
        );
        assert_eq!(
    ...
    fn test_check_payer_balances_distribute_tokens_single_payer() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (allocation_amount + fees_in_gth).to_string());
        } else {
    ...
    fn test_check_payer_balances_distribute_tokens_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, fees_in_gth.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    }
    ...
    fn test_check_payer_balances_distribute_stakes_single_payer() {
        ...
        let allocation_amount = 1000.0;
        let unlocked_gth = 1.0;
        let stake_args = initialize_stake_account(
            gth_to_weis(allocation_amount),
            gth_to_weis(unlocked_gth),
            &alice,
            &client,
        );
        ...
        let expensive_allocations = vec![Allocation {
            ...
            amount: gth_to_weis(expensive_allocation_amount),
            lockup_date: "".to_string(),
        }];
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(
                amount,
                (expensive_allocation_amount - unlocked_gth).to_string()
            );
        } else {
            panic!("check_payer_balances should have errored");
        }
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (unlocked_gth + fees_in_gth).to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    ...
    fn test_check_payer_balances_distribute_stakes_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            assert_eq!(sources, vec![FundingSource::SystemAccount].into());
            assert_eq!(amount, unlocked_gth.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
