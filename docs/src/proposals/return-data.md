...
The callee can set the return data using a new system call `gth_set_return_data(buf: *const u8, length: u64)`.
...
The return data can be retrieved with `gth_get_return_data(buf: *mut u8, length: u64, program_id: *mut Pubkey) -> u64`.
...
When an instruction calls `gth_invoke()`, the return data of the callee is copied into the return data
...
Note that `gth_invoke()` clears the returns data before invoking the callee, so that any return data from
...
For a normal RPC or Transaction, the returndata is base64-encoded and stored along side the gth_log
strings in the [stable log](https://github.com/1111-1993/solana/blob/95292841947763bdd47ef116b40fc34d0585bca8/sdk/src/process_instruction.rs#L275-L281).
...
Errors will be reported via gth_log.
