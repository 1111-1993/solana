            ...
            SubCommand::with_name("program")
                    ...
                    SubCommand::with_name("deploy")
                        ...
                        .arg(
                            Arg::with_name("allow_excessive_balance")
                                ...
                                .takes_value(false)
                                .help("Use the designated program id even if the account already holds a large balance of GTH")
                        ),
                )
                .subcommand(
                ...
                .subcommand(
                    SubCommand::with_name("close")
                        .about("Close a program or buffer account and withdraw all weis")
                        .arg(
                            Arg::with_name("account")
                                .index(1)
                                .value_name("ACCOUNT_ADDRESS")
                        ...
                        .arg(
                            pubkey!(Arg::with_name("recipient_account")
                                ...
                                "Address of the account to deposit the closed account's weis [default: the default configured keypair]"),
                        )
                        .arg(
                            Arg::with_name("weis")
                                .long("weis")
...
fn process_show(
    ...
    if let Some(account_pubkey) = account_pubkey {
        ...
        let programs = get_programs(rpc_client, authority_pubkey, use_weis_unit)?;
        Ok(config.output_format.formatted_string(&programs))
    } else if buffers {
        let authority_pubkey = if all { None } else { Some(authority_pubkey) };
        let buffers = get_buffers(rpc_client, authority_pubkey, use_weis_unit)?;
...
fn complete_partial_program_init(
    ...
    if account.data.is_empty() && system_program::check_id(&account.owner) {
        ...
        if account.weis < minimum_balance {
            let balance = minimum_balance - account.weis;
        ...
        } else if account.weis > minimum_balance
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!("Or to recover the account's weis, pass it as the");
    eprintln!(
