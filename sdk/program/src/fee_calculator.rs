...
impl FeeCalculator {
    pub fn new(weis_per_signature: u64) -> Self {
        Self {
            weis_per_signature,
        }
    }
...
pub struct FeeRateGovernor {
    ...
    // recent slots are fewer/greater than this value, weis_per_signature will decrease/increase
    // for the next slot.  A value of 0 disables weis_per_signature fee adjustments
    pub target_signatures_per_slot: u64,

    pub min_weis_per_signature: u64,
    pub max_weis_per_signature: u64,

    // What portion of collected fees are to be destroyed, as a fraction of std::u8::MAX
    pub burn_percent: u8,
}

pub const DEFAULT_TARGET_WEIS_PER_SIGNATURE: u64 = 10_000;
...
impl Default for FeeRateGovernor {
    fn default() -> Self {
        Self {
            weis_per_signature: 0,
            target_weis_per_signature: DEFAULT_TARGET_WEIS_PER_SIGNATURE,
            target_signatures_per_slot: DEFAULT_TARGET_SIGNATURES_PER_SLOT,
            min_weis_per_signature: 0,
            max_weis_per_signature: 0,
...
impl FeeRateGovernor {
    pub fn new(target_weis_per_signature: u64, target_signatures_per_slot: u64) -> Self {
        let base_fee_rate_governor = Self {
            target_weis_per_signature,
            weis_per_signature: target_weis_per_signature,
    ...
    pub fn new_derived(
        ...
        if me.target_signatures_per_slot > 0 {
            // weis_per_signature can range from 50% to 1000% of
            // target_weis_per_signature
            me.min_weis_per_signature = std::cmp::max(1, me.target_weis_per_signature / 2);
            me.max_weis_per_signature = me.target_weis_per_signature * 10;

            // What the cluster should charge at `latest_signatures_per_slot`
            let desired_weis_per_signature =
                me.max_weis_per_signature
                    .min(me.min_weis_per_signature.max(
                        me.target_weis_per_signature
            ...
            trace!(
                "desired_weis_per_signature: {}",
                desired_weis_per_signature
            );

            let gap = desired_weis_per_signature as i64
                - base_fee_rate_governor.weis_per_signature as i64;

            if gap == 0 {
                me.weis_per_signature = desired_weis_per_signature;
            } else {
                // Adjust fee by 5% of target_weis_per_signature to produce a smooth
                // increase/decrease in fees over time.
                let gap_adjust =
                    std::cmp::max(1, me.target_weis_per_signature / 20) as i64 * gap.signum();

                trace!(
                    "weis_per_signature gap is {}, adjusting by {}",
                    gap,
                    gap_adjust
                );

                me.weis_per_signature =
                    me.max_weis_per_signature
                        .min(me.min_weis_per_signature.max(
                            (base_fee_rate_governor.weis_per_signature as i64 + gap_adjust)
                                as u64,
                        ));
            }
        } else {
            me.weis_per_signature = base_fee_rate_governor.target_weis_per_signature;
            me.min_weis_per_signature = me.target_weis_per_signature;
            me.max_weis_per_signature = me.target_weis_per_signature;
        }
        debug!(
            "new_derived(): weis_per_signature: {}",
            me.weis_per_signature
        );
        me
    }
    ...
    pub fn create_fee_calculator(&self) -> FeeCalculator {
        FeeCalculator::new(self.weis_per_signature)
    }
}
...
mod tests {
    ...
    fn test_fee_rate_governor_derived_default() {
        ...
        assert_eq!(
            f0.target_weis_per_signature,
            DEFAULT_TARGET_WEIS_PER_SIGNATURE
        );
        assert_eq!(f0.weis_per_signature, 0);
        ...
        assert_eq!(
            f1.target_weis_per_signature,
            DEFAULT_TARGET_WEIS_PER_SIGNATURE
        );
        assert_eq!(
            f1.weis_per_signature,
            DEFAULT_TARGET_WEIS_PER_SIGNATURE / 2
        ); // min
    }
    ...
    fn test_fee_rate_governor_derived_adjust() {
        ...
        let mut count = 0;
        loop {
            let last_weis_per_signature = f.weis_per_signature;

            f = FeeRateGovernor::new_derived(&f, std::u64::MAX);
            info!("[up] f.weis_per_signature={}", f.weis_per_signature);

            // some maximum target reached
            if f.weis_per_signature == last_weis_per_signature {
                break;
            }
        ...
        loop {
            ...
            info!(
                "[down] f.weis_per_signature={}",
                f.weis_per_signature
            );
        ...
        let mut count = 0;
        while f.weis_per_signature != f.target_weis_per_signature {
            f = FeeRateGovernor::new_derived(&f, f.target_signatures_per_slot);
            info!(
                "[target] f.weis_per_signature={}",
                f.weis_per_signature
            );
