...
#[allow(deprecated)]
pub fn create_account_with_data<'a, I>(weis: u64, recent_blockhash_iter: I) -> AccountSharedData
where
    I: IntoIterator<Item = IterItem<'a>>,
{
    #[allow(deprecated)]
    create_account_with_data_and_fields(recent_blockhash_iter, (weis, INITIAL_RENT_EPOCH))
}
...
mod tests {
    ...
    fn test_create_account_full() {
        ...
        let def_weis_per_signature = 0;
        let account = create_account_with_data_for_test(
            vec![IterItem(0u64, &def_hash, def_weis_per_signature); MAX_ENTRIES].into_iter(),
        );
        let recent_blockhashes = from_account::<RecentBlockhashes, _>(&account).unwrap();
    ...
    fn test_create_account_truncate() {
        ...
        let def_weis_per_signature = 0;
        let account = create_account_with_data_for_test(
            vec![IterItem(0u64, &def_hash, def_weis_per_signature); MAX_ENTRIES + 1]
                .into_iter(),
        );
        let recent_blockhashes = from_account::<RecentBlockhashes, _>(&account).unwrap();
        assert_eq!(recent_blockhashes.len(), MAX_ENTRIES);
    ...
    fn test_create_account_unsorted() {
        ...
        let account = create_account_with_data_for_test(
            unsorted_blocks
                .iter()
                .map(|(i, hash)| IterItem(*i, hash, def_weis_per_signature)),
        );
        let recent_blockhashes = from_account::<RecentBlockhashes, _>(&account).unwrap();
