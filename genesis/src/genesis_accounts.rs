use {
    ...
    solana_sdk::{genesis_config::GenesisConfig, native_token::WEIS_PER_GTH},
};
...
pub const COMMUNITY_STAKER_INFOS: &[StakerInfo] = &[
    ...
    StakerInfo {
        ...
        weis: 153_333_633 * WEIS_PER_GTH + 41 * WEIS_PER_GTH / 100,
...
pub fn add_genesis_accounts(genesis_config: &mut GenesisConfig, mut issued_weis: u64) {
    ...
    // "one thanks" (community pool) gets 500_000_000GTH (total) - above distributions
    create_and_add_stakes(
        genesis_config,
        &StakerInfo {
            name: "one thanks",
            staker: "7vEAL3nS9CWmy1q6njUUyHE7Cf5RmyQpND6CsoHjzPiR",
            weis: (500_000_000 * WEIS_PER_GTH).saturating_sub(issued_weis),
...
mod tests {
    ...
    fn test_add_genesis_accounts() {
        ...
        let weis = genesis_config
            .accounts
            .iter()
            .map(|(_, account)| account.weis)
            .sum::<u64>();

        assert_eq!(500_000_000 * WEIS_PER_GTH, weis);
    }
}
