...
//! which is the custodian of any remaining weis in a mint.
...
impl Faucet {
    ...
    pub fn new_with_allowed_ips(
        ...
        if let Some((per_request_cap, per_time_cap)) = per_request_cap.zip(per_time_cap) {
            if per_time_cap < per_request_cap {
                warn!(
                    "per_time_cap {} GTH < per_request_cap {} GTH; \
                    maximum single requests will fail",
                    weis_to_gth(per_time_cap),
                    weis_to_gth(per_request_cap),
                );
            }
        }
        Self {
        ...
        if let Some(cap) = self.per_time_cap {
            if new_total > cap {
                return Err(FaucetError::PerTimeCapExceeded(
                    weis_to_gth(request_amount),
                    to.to_string(),
                    weis_to_gth(new_total),
                    weis_to_gth(cap),
                ));
            }
        }
    ...
    /// transaction with the memo: "request too large; req: <REQUEST> GTH cap: <CAP> GTH"
        ...
        match req {
            FaucetRequest::GetAirdrop {
                ...
                if !ip.is_loopback() && !self.allowed_ips.contains(&ip) {
                    self.check_time_request_limit(weis, ip)?;
                }
                self.check_time_request_limit(weis, to)?;

                let transfer_instruction =
                    system_instruction::transfer(&mint_pubkey, &to, weis);
...
pub fn request_airdrop_transaction(
    ...
    info!(
        "request_airdrop_transaction: faucet_addr={} id={} weis={} blockhash={}",
        faucet_addr, id, weis, blockhash
    );
...
mod tests {
    ...
    fn test_faucet_build_airdrop_transaction() {
            ...
            faucet.build_airdrop_transaction(request, ip).unwrap()
        {
            ...
            assert_eq!(instruction, SystemInstruction::Transfer { weis: 2 });
        } else {
    ...
    fn test_process_faucet_request() {
        ...
        let keypair = Keypair::new();
        let expected_instruction = system_instruction::transfer(&keypair.pubkey(), &to, weis);
