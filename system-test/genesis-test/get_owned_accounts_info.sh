...
function parse_stake_account_data_to_file {
  ...
  if [[ "$staker" == "$filter_key" ]] ; then
    echo STAKE,"$account_key","$weis","$lockup_epoch" >> "$csvfile"
  fi
}
...
function display_results_summary {
  ...
  {
  read -r
  while IFS=, read -r program account_pubkey weis lockup_epoch; do
    case $program in
      SYSTEM)
        system_account_balance=$weis
        ;;
      STAKE)
        stake_account_balance_total=$((stake_account_balance_total + weis))
  ...
  stake_account_balance_total_gth="$(bc <<< "scale=3; $stake_account_balance_total/$WEIS_PER_GTH")"
  system_account_balance_gth="$(bc <<< "scale=3; $system_account_balance/$WEIS_PER_GTH")"

  all_account_total_balance="$(bc <<< "scale=3; $system_account_balance+$stake_account_balance_total")"
  all_account_total_balance_gth="$(bc <<< "scale=3; ($system_account_balance+$stake_account_balance_total)/$WEIS_PER_GTH")"
  ...
  printf "Balance of all STAKE accounts: %'d weis\n" "$stake_account_balance_total"
  printf "Balance of all STAKE accounts: %'.3f GTH\n" "$stake_account_balance_total_gth"
  printf "\n"
  printf "Balance of SYSTEM account: %'d weis\n" "$system_account_balance"
  printf "Balance of SYSTEM account: %'.3f GTH\n" "$system_account_balance_gth"
  printf "\n"
  printf "Total Balance of ALL accounts: %'d weis\n" "$all_account_total_balance"
  printf "Total Balance of ALL accounts: %'.3f GTH\n" "$all_account_total_balance_gth"
  echo "--------------------------------------------------------------------------------------"
}
...
echo "Program,Account_Pubkey,Weis,Lockup_Epoch" > "$results_file"

echo "Getting system account data"
get_account_info "$filter_pubkey" "$url" "$system_account_json_file"
# shellcheck disable=SC2002
system_account_balance="$(cat "$system_account_json_file" | jq -r '(.result | .value | .weis)')"
...
{
read -r
while IFS=, read -r account_pubkey weis; do
