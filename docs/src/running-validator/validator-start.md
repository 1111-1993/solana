...
Airdrop yourself some GTH to get started:
...
here](https://github.com/1111-1993/solana/blob/36167b032c03fc7d1d8c288bb621920aaf903311/core/src/ledger_cleanup_service.rs#L23-L34).
...
Assuming you have a user called `gth` on your machine, create the file `/etc/systemd/system/gth.service` with
the following:
...
Environment="PATH=/bin:/usr/bin:/home/gth/.local/share/solana/install/active_release/bin"
ExecStart=/home/gth/bin/validator.sh

[Install]
WantedBy=multi-user.target
```

Now create `/home/gth/bin/validator.sh` to include the desired
...
Ensure that running `/home/gth/bin/validator.sh` manually starts
the validator as expected. Don't forget to mark it executable with `chmod +x /home/gth/bin/validator.sh`

Start the service with:

```bash
$ sudo systemctl enable --now gth
```
...
running as a systemd service called `gth.service` and writes a log file at
/home/gth/solana-validator.log:

```bash
# Setup log rotation

cat > logrotate.gth <<EOF
/home/gth/solana-validator.log {
  rotate 7
  daily
  missingok
  postrotate
    systemctl kill -s USR1 gth.service
  endscript
}
EOF
sudo cp logrotate.gth /etc/logrotate.d/gth
...
2. Add a 300GB tmpfs parition by adding a new line containing `tmpfs /mnt/solana-accounts tmpfs rw,size=300G,user=gth 0 0` to `/etc/fstab`
   (assuming your validator is running under the user "gth"). **CAREFUL: If you
