...
/// # let tx = system_transaction::transfer(&key, &to, weis, latest_blockhash);
...
impl RpcClient {
    ...
    /// let tx = system_transaction::transfer(&alice, &bob.pubkey(), weis, latest_blockhash);
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// // Transfer weis from Alice to Bob
    /// # let alice = Keypair::new();
    /// # let bob = Keypair::new();
    /// # let weis = 50;
    /// let latest_blockhash = rpc_client.get_latest_blockhash()?;
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// // Transfer weis from Alice to Bob and wait for confirmation
    /// # let alice = Keypair::new();
    /// # let bob = Keypair::new();
    /// # let weis = 50;
    /// let latest_blockhash = rpc_client.get_latest_blockhash()?;
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// // Send weis from Alice to Bob and wait for the transaction to be processed
    /// # let bob = Keypair::new();
    /// # let weis = 50;
    /// let latest_blockhash = rpc_client.get_latest_blockhash()?;
    /// let tx = system_transaction::transfer(&alice, &bob.pubkey(), weis, latest_blockhash);
    ...
    /// Returns the 20 largest accounts, by wei balance.
    ///
    /// # RPC Reference
    ///
    ...
    pub fn request_airdrop(&self, pubkey: &Pubkey, weis: u64) -> ClientResult<Signature> {
        self.invoke(self.rpc_client.request_airdrop(pubkey, weis))
    }
    ...
    pub fn request_airdrop_with_config(
        ...
        self.invoke(
            self.rpc_client
                .request_airdrop_with_config(pubkey, weis, config),
        )
    }
