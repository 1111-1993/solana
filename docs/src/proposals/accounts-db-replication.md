...
up due to zero weis, i.e. we need to be able to tell the difference between an account in a
given slot which was not updated and hence has no storage entry in that slot, and one that
holds 0 weis and has been cleaned up through the history. We may record this via some
