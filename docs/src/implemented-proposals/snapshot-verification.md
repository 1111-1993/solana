...
On account store of non-zero wei accounts, we hash the following data:

- Account owner
- Account data
- Account pubkey
- Account weis balance
...
A snapshot must be purged of zero wei accounts before creation and during verify since the zero wei accounts do not affect the hash value but may cause
