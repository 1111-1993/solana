...
const DEFAULT_FAUCET_GTH: f64 = 1_000_000.;
...
fn main() {
    ...
    let default_faucet_gth = DEFAULT_FAUCET_GTH.to_string();
    ...
    let matches = App::new("solana-test-validator")
        ...
        .arg(
            Arg::with_name("faucet_gth")
                .long("faucet-gth")
                .takes_value(true)
                .value_name("GTH")
                .default_value(default_faucet_gth.as_str())
                .help(
                    "Give the faucet address this much GTH in genesis. \
                     If the ledger already exists then this parameter is silently ignored",
                ),
        )
        .arg(
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `solana airdrop` localnet GTH after creating one\n",
        );
    }
