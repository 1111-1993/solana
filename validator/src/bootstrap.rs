...
fn check_vote_account(
    ...
    // Maybe we can calculate minimum voting fee; rather than 1 wei
    if identity_account.weis <= 1 {
        return Err(format!(
            "underfunded identity account ({}): only {} weis available",
            identity_pubkey, identity_account.weis
        ));
    }
