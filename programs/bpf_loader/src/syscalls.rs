...
use {
    ...
    solana_sdk::{
        ...
        feature_set::{
            ...
            gth_log_data_syscall_enabled, syscall_saturated_math, update_syscall_base_costs,
        },
...
pub fn register_syscalls(
    ...
    syscall_registry.register_syscall_by_name(b"gth_panic_", SyscallPanic::call)?;
    syscall_registry.register_syscall_by_name(b"gth_log_", SyscallLog::call)?;
    syscall_registry.register_syscall_by_name(b"gth_log_64_", SyscallLogU64::call)?;

    syscall_registry
        .register_syscall_by_name(b"gth_log_compute_units_", SyscallLogBpfComputeUnits::call)?;

    syscall_registry.register_syscall_by_name(b"gth_log_pubkey", SyscallLogPubkey::call)?;

    syscall_registry.register_syscall_by_name(
        b"gth_create_program_address",
        SyscallCreateProgramAddress::call,
    )?;
    syscall_registry.register_syscall_by_name(
        b"gth_try_find_program_address",
        SyscallTryFindProgramAddress::call,
    )?;

    syscall_registry.register_syscall_by_name(b"gth_sha256", SyscallSha256::call)?;
    syscall_registry.register_syscall_by_name(b"gth_keccak256", SyscallKeccak256::call)?;
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"gth_secp256k1_recover", SyscallSecp256k1Recover::call)?;
    }
    ...
    {
        syscall_registry.register_syscall_by_name(b"gth_blake3", SyscallBlake3::call)?;
    }
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"gth_zk_token_elgamal_op", SyscallZkTokenElgamalOp::call)?;
        syscall_registry.register_syscall_by_name(
            b"gth_zk_token_elgamal_op_with_lo_hi",
            SyscallZkTokenElgamalOpWithLoHi::call,
        )?;
        syscall_registry.register_syscall_by_name(
            b"gth_zk_token_elgamal_op_with_scalar",
            SyscallZkTokenElgamalOpWithScalar::call,
        )?;
    }

    syscall_registry
        .register_syscall_by_name(b"gth_get_clock_sysvar", SyscallGetClockSysvar::call)?;
    syscall_registry.register_syscall_by_name(
        b"gth_get_epoch_schedule_sysvar",
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"gth_get_fees_sysvar", SyscallGetFeesSysvar::call)?;
    }
    syscall_registry
        .register_syscall_by_name(b"gth_get_rent_sysvar", SyscallGetRentSysvar::call)?;

    syscall_registry.register_syscall_by_name(b"gth_memcpy_", SyscallMemcpy::call)?;
    syscall_registry.register_syscall_by_name(b"gth_memmove_", SyscallMemmove::call)?;
    syscall_registry.register_syscall_by_name(b"gth_memcmp_", SyscallMemcmp::call)?;
    syscall_registry.register_syscall_by_name(b"gth_memset_", SyscallMemset::call)?;

    // Cross-program invocation syscalls
    syscall_registry
        .register_syscall_by_name(b"gth_invoke_signed_c", SyscallInvokeSignedC::call)?;
    syscall_registry
        .register_syscall_by_name(b"gth_invoke_signed_rust", SyscallInvokeSignedRust::call)?;

    // Memory allocator
    syscall_registry.register_syscall_by_name(b"gth_alloc_free_", SyscallAllocFree::call)?;
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"gth_set_return_data", SyscallSetReturnData::call)?;
        syscall_registry
            .register_syscall_by_name(b"gth_get_return_data", SyscallGetReturnData::call)?;
    }

    // Log data
    if invoke_context
        .feature_set
        .is_active(&gth_log_data_syscall_enabled::id())
    {
        syscall_registry.register_syscall_by_name(b"gth_log_data", SyscallLogData::call)?;
    }
    ...
    {
        syscall_registry.register_syscall_by_name(
            b"gth_get_processed_sibling_instruction",
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"gth_get_stack_height", SyscallGetStackHeight::call)?;
    }
...
pub fn bind_syscall_context_objects<'a, 'b>(
    ...
    let is_return_data_syscall_active = invoke_context
        .feature_set
        .is_active(&return_data_syscall_enabled::id());
    let is_gth_log_data_syscall_active = invoke_context
        .feature_set
        .is_active(&gth_log_data_syscall_enabled::id());
    ...
    bind_feature_gated_syscall_context_object!(
        vm,
        is_gth_log_data_syscall_active,
...
/// Panic syscall function, called when the BPF program calls 'gth_panic_()`
/// Causes the BPF program to be halted immediately
...
/// `gth_alloc_free_()`.  The allocator is expected to allocate/free
...
/// Rust representation of C's GthInstruction
#[derive(Debug)]
#[repr(C)]
struct GthInstruction {
    program_id_addr: u64,
    accounts_addr: u64,
    accounts_len: u64,
    data_addr: u64,
...
/// Rust representation of C's GthAccountMeta
#[derive(Debug)]
#[repr(C)]
struct GthAccountMeta {
    pubkey_addr: u64,
    is_writable: bool,
    is_signer: bool,
}

/// Rust representation of C's GthAccountInfo
#[derive(Debug)]
#[repr(C)]
...
impl<'a, 'b> SyscallInvokeSigned<'a, 'b> for SyscallInvokeSignedC<'a, 'b> {
    ...
    fn translate_instruction(
        ...
        let ix_c = translate_type::<GthInstruction>(memory_mapping, addr, loader_id)?;
        ...
        let meta_cs = translate_slice::<GthAccountMeta>(
            memory_mapping,
            ix_c.accounts_addr,
    ...
    fn translate_accounts<'c>(
        ...
        let account_infos = translate_slice::<GthAccountInfo>(
            memory_mapping,
            account_infos_addr,
            account_infos_len,
            loader_id,
...
mod tests {
    ...
    fn test_syscall_gth_log() {
        ...
        let mut syscall_gth_log = SyscallLog {
            invoke_context: Rc::new(RefCell::new(&mut invoke_context)),
        };
    ...
    fn test_syscall_gth_log_u64() {
        ...
        let cost = invoke_context.get_compute_budget().log_64_units;
        let mut syscall_gth_log_u64 = SyscallLogU64 {
            invoke_context: Rc::new(RefCell::new(&mut invoke_context)),
        };
        ...
        let mut result: Result<u64, EbpfError<BpfError>> = Ok(0);
        syscall_gth_log_u64.call(1, 2, 3, 4, 5, &memory_mapping, &mut result);
        result.unwrap();
    ...
    fn test_syscall_gth_pubkey() {
        ...
        let cost = invoke_context.get_compute_budget().log_pubkey_units;
        let mut syscall_gth_pubkey = SyscallLogPubkey {
            invoke_context: Rc::new(RefCell::new(&mut invoke_context)),
        };
        ...
        let mut result: Result<u64, EbpfError<BpfError>> = Ok(0);
        syscall_gth_pubkey.call(100, 32, 0, 0, 0, &memory_mapping, &mut result);
        assert_eq!(
            Err(EbpfError::UserError(BpfError::SyscallError(
                SyscallError::InstructionError(InstructionError::ComputationalBudgetExceeded)
            ))),
        ...
        syscall_gth_pubkey
            ...
            .mock_set_remaining(cost);
        let mut result: Result<u64, EbpfError<BpfError>> = Ok(0);
        syscall_gth_pubkey.call(0x100000000, 0, 0, 0, 0, &memory_mapping, &mut result);
        result.unwrap();
    ...
    #[test]
    fn test_syscall_gth_alloc_free() {
        let config = Config::default();
        // large alloc
        {
