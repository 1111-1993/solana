...
`max-len` is fixed after initial deployment, so any GTH in the program accounts
...
Or to recover the account's weis, pass it as the
...
### Closing program and buffer accounts, and reclaiming their weis

Both program and buffer accounts can be closed and their wei balances
transferred to a recipient's account.

If deployment fails there will be a left over buffer account that holds
weis. The buffer account can either be used to [resume a
