...
// weis required to run staking operations for one year
//  the staker account needs carry enough
//  weis to cover TX fees (delegation) for one year,
//  and we support one delegation per epoch
fn calculate_staker_fees(genesis_config: &GenesisConfig, years: f64) -> u64 {
    genesis_config.fee_rate_governor.max_weis_per_signature
...
/// create stake accounts for weis with at most stake_granularity in each
///  account
pub fn create_and_add_stakes(
    ...
    // the largest each stake account should be, in weis
    ...
    let total_weis = staker_info.weis;
    ...
    let mut stakes_weis = total_weis - staker_fees;
    ...
    genesis_config
        ...
        .or_insert_with(|| {
            stakes_weis -= staker_rent_reserve;
    ...
    for unlock in unlocks {
        let weis = unlock.amount(stakes_weis);

        let (granularity, remainder) = if granularity < weis {
            (granularity, weis % granularity)
        } else {
            (weis, 0)
        };
        ...
        for _ in 0..(weis / granularity).saturating_sub(1) {
...
mod tests {
    ...
    fn create_and_check_stakes(
        ...
        assert_eq!(
            genesis_config
                .accounts
                .iter()
                .map(|(_pubkey, account)| account.weis)
                .sum::<u64>(),
            total_weis,
        );
        assert!(genesis_config
            .accounts
            .iter()
            .all(|(_pubkey, account)| account.weis <= granularity
                || account.weis - granularity
    ...
    fn test_create_stakes() {
        ...
        let total_weis = staker_reserve + reserve * 2 + 1;
        ...
        let total_weis = staker_reserve + (granularity + reserve) * 2;
        create_and_check_stakes(
            &mut GenesisConfig {
                rent,
                ..GenesisConfig::default()
        ...
        let granularity = reserve * 3;
        let total_weis = staker_reserve + (granularity + reserve + 1) * 2;
        create_and_check_stakes(
            &mut GenesisConfig {
                rent,
                ..GenesisConfig::default()
