...
mod tests {
    ...
    /// Test that cleaning works well in the edge cases of zero-wei accounts and snapshots.
    /// Here's the scenario:
    ///
    /// slot 1:
    ///     - send some weis to Account1 (from Account2) to bring it to life
    ///     - take a full snapshot
    /// slot 2:
    ///     - make Account1 have zero weis (send back to Account2)
    ...
    /// no longer correct!
    #[test]
    fn test_incremental_snapshots_handle_zero_wei_accounts() {
        solana_logger::setup();
        ...
        bank0
            .transfer(weis_to_transfer, &mint_keypair, &key2.pubkey())
            .unwrap();
        while !bank0.is_complete() {
            bank0.register_tick(&Hash::new_unique());
        }
        ...
        let bank1 = Arc::new(Bank::new_from_parent(&bank0, &collector, slot));
        bank1
            .transfer(weis_to_transfer, &key2, &key1.pubkey())
            .unwrap();
        while !bank1.is_complete() {
            bank1.register_tick(&Hash::new_unique());
        }
