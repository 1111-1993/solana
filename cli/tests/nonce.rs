...
fn full_battery_tests(
    ...
    config_payer.command = CliCommand::CreateNonceAccount {
        ...
        memo: None,
        amount: SpendAmount::Some(gth_to_weis(1000.0)),
    };
    ...
    check_balance!(gth_to_weis(1000.0), &rpc_client, &nonce_account);
    ...
    check_balance!(gth_to_weis(900.0), &rpc_client, &nonce_account);
    check_balance!(gth_to_weis(100.0), &rpc_client, &payee_pubkey);
    ...
    check_balance!(gth_to_weis(800.0), &rpc_client, &nonce_account);
    check_balance!(gth_to_weis(200.0), &rpc_client, &payee_pubkey);
}
...
fn test_create_account_with_seed() {
    ...
    creator_config.command = CliCommand::CreateNonceAccount {
        ...
        memo: None,
        amount: SpendAmount::Some(gth_to_weis(241.0)),
    };
    process_command(&creator_config).unwrap();
    check_balance!(gth_to_weis(241.0), &rpc_client, &nonce_address);
    check_balance!(
        gth_to_weis(42.0),
        &rpc_client,
        &offline_nonce_authority_signer.pubkey(),
    ...
    check_balance!(
        gth_to_weis(4001.0 - ONE_SIG_FEE),
        &rpc_client,
        &online_nonce_creator_signer.pubkey(),
    );
    check_balance!(0, &rpc_client, &to_address);
    ...
    authority_config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(gth_to_weis(10.0)),
        to: to_address,
        from: 0,
        sign_only: true,
    ...
    check_balance!(gth_to_weis(10.0), &rpc_client, &to_address);
}
