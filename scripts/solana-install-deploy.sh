...
edge|beta)
  ...
  ;;
*)
  DOWNLOAD_URL=https://github.com/1111-1993/solana/releases/download/"$TAG"/solana-release-$TARGET.tar.bz2
  ;;
esac
...
balance=$(solana $maybeKeypair --url "$URL" balance --weis)
if [[ $balance = "0 weis" ]]; then
  # shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
