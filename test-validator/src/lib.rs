...
impl TestValidator {
    ...
    pub fn with_custom_fees(
        mint_address: Pubkey,
        target_weis_per_signature: u64,
        faucet_addr: Option<SocketAddr>,
        socket_addr_space: SocketAddrSpace,
    ) -> Self {
        TestValidatorGenesis::default()
            .fee_rate_governor(FeeRateGovernor::new(target_weis_per_signature, 0))
            .rent(Rent {
                weis_per_byte_year: 1,
                exemption_threshold: 1.0,
                ..Rent::default()
    ...
    fn initialize_ledger(
        ...
        let validator_identity_weis = gth_to_weis(500.);
        let validator_stake_weis = gth_to_weis(1_000_000.);
        let mint_weis = gth_to_weis(500_000_000.);
