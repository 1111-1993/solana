...
pub enum UpgradeableLoaderInstruction {
    ...
    /// set to `None`.
    ///
    /// The Buffer account must contain sufficient weis to fund the
    /// ProgramData account to be rent-exempt, any additional weis left over
    ...
    /// Closes an account owned by the upgradeable loader of all weis and
    /// withdraws all the weis
    ///
    /// # Account references
    ///   0. `[writable]` The account to close, if closing a program must be the
    ///      ProgramData account.
    ///   1. `[writable]` The account to deposit the closed account's weis.
