...
tokens called _weis_. Accounts are held in validator memory and pay
["rent"](#rent) to stay there. Each validator periodically scans all accounts
and collects rent. Any account that drops to zero weis is purged. Accounts
...
previously created, the program will be passed an account with no data and zero weis
...
System program, the runtime allows clients to transfer weis and importantly
...
deserialization](https://github.com/1111-1993/solana/blob/a95675a7ce1651f7b59443eb146b356bc4b3f374/sdk/program/src/sysvar/mod.rs#L65).
An alternative and safer way to read a sysvar is via the sysvar's [`get()`
function](https://github.com/1111-1993/solana/blob/64bfc14a75671e4ec3fe969ded01a599645080eb/sdk/program/src/sysvar/mod.rs#L73)
...
balance of 105,290,880 weis (=~ 0.105 GTH) to be rent-exempt:
...
```text
$ solana rent 15000
Rent per byte-year: 0.00000348 GTH
Rent per epoch: 0.000288276 GTH
Rent-exempt minimum: 0.10529088 GTH
```
