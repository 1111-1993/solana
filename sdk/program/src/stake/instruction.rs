...
pub enum StakeInstruction {
    ...
    /// Withdraw unstaked weis from the stake account
    ///
    /// # Account references
    ...
    ///    must be `<= StakeAccount.weis - staked_weis`.
    Withdraw(u64),
...
fn _split(
    ...
    Instruction::new_with_bincode(id(), &StakeInstruction::Split(weis), account_metas)
}
...
pub fn create_account_and_delegate_stake(
    ...
    let mut instructions = create_account(from_pubkey, stake_pubkey, authorized, lockup, weis);
...
pub fn withdraw(
    ...
    Instruction::new_with_bincode(id(), &StakeInstruction::Withdraw(weis), account_metas)
}
