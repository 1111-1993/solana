...
airdrops_enabled=1
node_gth=500 # 500 GTH: number of GTH to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-gth GTH            - Number of GTH this node has been funded from the genesis config (default: $node_gth)
...
setup_validator_accounts() {
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      echo "Adding $node_gth to validator identity account:"
      (
        set -x
        $solana_cli \
          --keypair "$SOLANA_CONFIG_DIR/faucet.json" --url "$rpc_url" \
          transfer --allow-unfunded-recipient "$identity" "$node_gth"
      ) || return $?
    fi
...
setup_validator_accounts "$node_gth"
