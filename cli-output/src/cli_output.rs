    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} GTH", lamports_to_gth(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} GTH", lamports_to_gth(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} GTH", lamports_to_gth(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} GTH", lamports_to_gth(self.non_circulating)),
        )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !self.encoded_confirmed_block.rewards.is_empty() {
            ...
            writeln!(
                ...
                "Total Rewards: {}◎{:<12.9}",
                sign,
                lamports_to_gth(total_rewards.abs() as u64)
            )?;
        }
