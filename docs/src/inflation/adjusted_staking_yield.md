...
This can be seen by explicitly calculating un-staked dilution as $D_{us}$. The un-staked proportion of the token pool at time $t$ is $P_{us}(t_{N})$ and $I_{t}$ is the incremental inflation rate applied between any two consecutive time points. $GTH_{us}(t)$ and $GTH_{total}(t)$ is the amount of un-staked and total GTH on the network, respectively, at time $t$. Therefore $P_{us}(t) = GTH_{us}(t)/GTH_{total}(t)$.

$$
\begin{aligned}
	D_{us} &= \left( \frac{P_{us}(t_{1}) - P_{us}(t_{0})}{P_{us}(t_{0})} \right)\\
		&= \left( \frac{ \left( \frac{GTH_{us}(t_{2})}{GTH_{total}(t_{2})} \right) - \left( \frac{GTH_{us}(t_{1})}{GTH_{total}(t_{1})} \right)}{ \left( \frac{GTH_{us}(t_{1})}{GTH_{total}(t_{1})} \right) } \right)\\
...
$$
\begin{aligned}
	GTH_{us}(t_2) &= GTH_{us}(t_1)\\
	GTH_{total}(t_2) &= GTH_{total}(t_1)\times (1 + I_{t_1})\\
\end{aligned}
$$
...
$$
\begin{aligned}
	D_{us} &= \left( \frac{ \left( \frac{GTH_{us}(t_{1})}{GTH_{total}(t_{1})\times (1 + I_{1})} \right) - \left( \frac{GTH_{us}(t_{1})}{GTH_{total}(t_{1})} \right)}{ \left( \frac{GTH_{us}(t_{1})}{GTH_{total}(t_{1})} \right) } \right)\\
...
As seen in the plot above, the proportion of staked tokens increases with inflation issuance. Letting $GTH_s(t)$ and $GTH_{\text{total}}(t)$ represent the amount of staked and total GTH at time $t$ respectively:

$$
	P_s(t_2) = \frac{GTH_s(t_1) + GTH_{\text{total}}(t_1)\times I(t_1)}{GTH_{\text{total}}(t_1)\times (1 + I(t_1))}\\
$$

Where $GTH_{\text{total}}(t_1)\times I(t_1)$ is the additional inflation issuance added to the staked token pool. Now we can write $Y_{adj}$ in common terms $t_1 = t$:

$$
\begin{aligned}
Y_{adj} &= \frac{\frac{GTH_s(t) + GTH_{\text{total}}(t)\times I(t)}{GTH_{\text{total}}(t)\times (1 + I(t))} - \frac{GTH_s(t)}{GTH_{\text{total}}(t)} }{ \frac{GTH_s(t)}{GTH_{\text{total}}(t)} }  \\
	&= \frac{ GTH_{\text{total}}(t)\times (GTH_s(t) + GTH_{\text{total}}(t)\times I(t)) }{ GTH_s(t)\times GTH_{\text{total}}\times (1 + I(t)) } -1 \\
\end{aligned}
$$
