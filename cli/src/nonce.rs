    ...
    fn nonce_subcommands(self) -> Self {
            ...
            SubCommand::with_name("create-nonce-account")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .help("The amount to load the nonce account with, in GTH; accepts keyword ALL"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("withdraw-from-nonce-account")
                .about("Withdraw GTH from the nonce account")
                .arg(
                    pubkey!(Arg::with_name("nonce_account_pubkey")
                        .index(1)
                        .value_name("NONCE_ACCOUNT_ADDRESS")
                ...
                .arg(
                    pubkey!(Arg::with_name("destination_account_pubkey")
                        ...
                        .required(true),
                        "The account to which the GTH should be transferred. "),
                )
                .arg(
                    Arg::with_name("amount")
                        .index(3)
                        ...
                        .required(true)
                        .validator(is_amount)
                        .help("The amount to withdraw from the nonce account, in GTH"),
                )
                .arg(nonce_authority_arg())
                .arg(memo_arg()),
        )
...
pub fn process_create_nonce_account(
    ...
    let (message, weis) = resolve_spend_tx_and_check_account_balance(
        rpc_client,
        false,
        amount,
        &latest_blockhash,
    ...
    if weis < minimum_balance {
        return Err(CliError::BadParameter(format!(
            "need at least {} weis for nonce account to be rent exempt, provided weis: {}",
            minimum_balance, weis
        ))
        .into());
    }
...
pub fn process_show_nonce_account(
    ...
    let print_account = |data: Option<&nonce::state::Data>| {
        ...
        if let Some(data) = data {
            ...
            nonce_account.weis_per_signature = Some(data.fee_calculator.weis_per_signature);
