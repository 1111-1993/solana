...
6. Precompiled programs
   - Precompiled programs are performing compute-intensive operations.  The work
     ...
     [#19627](https://github.com/1111-1993/solana/issues/19627)
...
https://github.com/1111-1993/solana/issues/20511
...
The precompiled
[ComputeBudget](https://github.com/1111-1993/solana/blob/00929f836348d76cb3503d0ba5f76f0d275bcc66/sdk/src/compute_budget.rs#L34)
...
https://github.com/1111-1993/solana/issues/20481
...
- blockhash queue contains a list of recent (<=~2min) blockhashes and a
  `weis_per_signature` value.  The blockhash queue is one of the snapshot's
  serialized members and thus bank hash depends on it.
- Nonce accounts used for offline signing contain a `weis_per_signature`
  value in its account data

In both cases, when a transaction is assessed a fee, the
`weis_per_signature` to use is looked up (either in the queue or in the
...
- Exposing the `FeeCalculator` object to the clients (holds the
  `weis_per_signature`) makes it hard to evolve the fee criteria due to
