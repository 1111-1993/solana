...
// `RpcGthPubSubInternal` is actually used by the current PubSub API implementation.
// `RpcGthPubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcGthPubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcGthPubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcGthPubSubInternal for RpcGthPubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
