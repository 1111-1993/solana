...
WithdrawInstruction(to, weis)
  if state == Uninitialized
    ...
    if !sysvar.recent_blockhashes.contains(stored_nonce)
      error NotReady
    if weis != account.balance && weis + rent_exempt > account.balance
      error InsufficientFunds
  account.balance -= weis
  to.balance += weis
  success
```
...
requirements of the feature, and as such, require that sufficient weis be
...
The `WithdrawNonceAccount` instruction takes a single argument, weis to withdraw,
...
would be zero weis, which makes the account eligible for deletion. This
