...
stake_gth=1   # default number of GTH to assign as stake (1 GTH)
...
usage: $0 [OPTIONS] <GTH to stake ($stake_gth)>
...
if ((airdrops_enabled)); then
  ...
  $solana_cli \
    ...
    transfer --allow-unfunded-recipient "$keypair" "$stake_gth"
fi
...
$solana_cli "${common_args[@]}" \
  create-stake-account "$stake_account" "$stake_gth"
$solana_cli "${common_args[@]}" \
